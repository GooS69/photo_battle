...
class PostView(APIView):
    ...
    def put(self, request, *args, **kwargs):
        outcome = ServiceOutcome(UpdatePostService, {'user': request.user, 'post_id': kwargs['pk'],
                                                     'name': request.data.get('name')}, request.FILES)
        if bool(outcome.errors):
            return Response(outcome.errors, outcome.response_status or status.HTTP_400_BAD_REQUEST)
        return Response(PostListSerializer(outcome.result).data, status=status.HTTP_200_OK)

    @swagger_auto_schema(request_body=ParticleUpdatePostSerializer, responses={200: 'ok'})
    def patch(self, request, *args, **kwargs):
        outcome = ServiceOutcome(ParticleUpdatePostService, {'user': request.user, 'post_id': kwargs['pk'],
                                                             'name': request.data.get('name'),
                                                             'status': request.data.get('status')}, request.FILES)
        if bool(outcome.errors):
            return Response(outcome.errors, outcome.response_status or status.HTTP_400_BAD_REQUEST)
        return Response(PostListSerializer(outcome.result).data, status=status.HTTP_200_OK)
